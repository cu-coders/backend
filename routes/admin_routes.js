...
router.get("/add-events", verifyAdminAuth,
           (req, res) => { res.render("./add-events"); });

// Saves form to the database
router.post("/add-events", verifyAdminAuth, uploadImage.single("cover"),
            async (req, res) => {
              try {
                const result =
...
router.get("/add-team", verifyAdminAuth,
           (req, res) => { res.render("add-team"); });

// Render the resources submission form
router.get("/add-resources", verifyAdminAuth,
           (req, res) => { res.render("add-resources"); });

// Handle team submit form request
router.post("/add-team", verifyAdminAuth, uploadImage.single("profileImage"),
            async (req, res) => {
              try {
                const result =
                    await cloudinaryConfig.uploader.upload(req.file.path, {
...
router.post(
    "/add-resources", verifyAdminAuth, uploadDoc.single("resources"),
    uploadImage.single("imageSrc"), async (req, res) => {
      try {
