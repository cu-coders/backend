...
const logger = winston.createLogger({
  ...
  transports : [
    new winston.transports.File(
        {filename : "membership_error.log"}), // Log membership errors to a file
  ],
});

exports.insertMembership = async (req, res) => {
  let uploadResult; // Declare the variable outside the try-catch to ensure
                    // access in the catch block
  ...
  try {
    ...
    if (!email || !fullname || !collegename || !collegeid || !branch || !year ||
        !phonenumber) {
      return res.status(400).json({
        success : false,
    ...
    return res.status(500).json({
      success : false,
      message :
          "An error occurred while processing your membership application.",
    });
  }
};
